...
const useDepth = (ref) => {
  ...
  const opacity = useTransform(scrollYProgress, [0, 0.3, 1], [0, 1, 0.95]);
...
const BackgroundEffects = () => {
  ...
  const scaleBg = useTransform(scrollY, [0, 5000], [1, 1.2]);
...
const Hero = ({ t }) => {
  ...
  const { scale, opacity, y } = useDepth(containerRef);
  const { scale: s, opacity: o, y: yy } = useDepth(containerRef);
  ...
  return (
    <section ref={containerRef} className="relative min-h-screen flex items-center pt-32 px-6 overflow-hidden"><motion.section
  ref={containerRef}
  ...
  return (
    <section id="концепція" className="py-40 px-6 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className={`glass-card hover-depth p-12 rounded-[4rem] relative overflow-hidden ${i % 2 === 0 ? 'bg-white/60 border-white/50' : 'bg-accent-purple/10 border-accent-purple/20'}`}>
